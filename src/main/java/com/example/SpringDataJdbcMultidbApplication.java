...
public class SpringDataJdbcMultidbApplication {
	...
	CommandLineRunner demoDb1(ProductDb1Repository repository) {
		return (args) -> {
			...
			Product p1 = new Product(null, "Gaming Mouse db1", new BigDecimal("1500.00"), LocalDateTime.now());
			Product p2 = new Product(null, "Mechanical Keyboard db1", new BigDecimal("3500.00"), LocalDateTime.now());
	...
	CommandLineRunner demoDb2(ProductDb2Repository repository) {
		return (args) -> {
			// == 1. Insert ข้อมูล ==
			// ตอน Insert ให้ส่ง ID เป็น null ไปก่อน เพื่อให้ DB รัน Auto-increment เอง
			Product p1 = new Product(null, "Gaming Mouse db2", new BigDecimal("1500.00"), LocalDateTime.now());
			Product p2 = new Product(null, "Mechanical Keyboard db2", new BigDecimal("3500.00"), LocalDateTime.now());
